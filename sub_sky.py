...
def sub_median(sky_file, img_file, output, mask_limit, cut_ch=300):
    ...
    for j in np.arange(cut_ch, (len(img[:,1,1])-cut_ch), 1):
        ...
        filtered_data.mask[sky[j,:,:]==0] = True
...
def combine(image1,image2,output):
    ...
    from astropy.io import fits
    import numpy as np
    img1, ih1 = fits.getdata('analysis/'+image1+'.fits', header=True)
    img2, ih2 = fits.getdata('analysis/'+image2+'.fits', header=True)
    var1, vh1 = fits.getdata('analysis/'+image1+'_var.fits', header=True)
    var2, vh2 = fits.getdata('analysis/'+image2+'_var.fits', header=True)
    ...
    img2_shift= cube_shift(img2,shift_x,shift_y)
    var2_shift= cube_shift(var2,shift_x,shift_y)

    err1=err(var1)
    err2_shift=err(var2_shift)

    ## direct stack, not good
    #img_comb=(img1+img2_shift)/2
    #err_comb=err1

    # weight: 1/err**2
    sum1=1/(err1**2)+1/(err2_shift**2) #sum of 1/err**2
    img_comb=(img1/(err1**2)+img2_shift/(err2_shift**2))/sum1
    err_comb=1/np.sqrt(sum1)

    var_comb=err_comb**2
    fits.writeto('analysis/'+output+'.fits', img_comb, ih1, overwrite=True)
    fits.writeto('analysis/'+output+'_var.fits', var_comb,ih1, overwrite=True)